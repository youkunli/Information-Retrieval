...
public:
	...
	binary_vectorPosting* not_operator(binary_vectorPosting* right) // not操作
	{
	...
	binary_vectorPosting* and_operator(binary_vectorPosting* right, int distance = 0) // and操作
	{
		binary_vectorPosting* ans = (new binary_vectorPosting());
	...
	binary_vectorPosting* or_operator(binary_vectorPosting* right) // or操作
	{
		binary_vectorPosting* ans = (new binary_vectorPosting());
...
class VectorPosting // vector实现的倒排记录表，查询时使用
{
private:
	vector<Post> posting;
	bool isNot;
...
public:
	...
	void push_back(Post post) // 插入一个倒排记录
	{
	...
	VectorPosting& not_operator(VectorPosting& right) // not操作
	{
	...
	VectorPosting& and_operator(VectorPosting& right, int distance = 0) // and操作
	{
		VectorPosting& ans = *(new VectorPosting());
		int i = 0, j = 0;
		int ll = length(), rr = right.length();
	...
	VectorPosting& or_operator(VectorPosting& right) // or操作
	{
		VectorPosting& ans = *(new VectorPosting());
		int i = 0, j = 0;
		int ll = length(), rr = right.length();
...
public:
	...
	void readIndex(string path) // 读入索引
	{
		string line;
		ifstream fin;
	...
	VectorPosting& find(string key) // 查找单词对应的倒排链
	{
	...
	void build(vector<string>& inFileName) // 建立索引
	{
	...
	void getDoc(VectorPosting& posting) // 文档id对应到文档名
	{
...
//cout << w << endl;
	...
	Node mergeNode(vector<Node> v) // 无括号的解析
	{
		Node* temp = new Node();
		if(v.size() == 0)
		{
	...
	Node orNode(vector<Node> v) // 短的倒排表， 先做OR操作
	{
