...
mLeafNode* BPlusTree::SearchLeafNode(const KeyType & data)const{
	...
	mNode * pNode = mRoot;
	{
		this ->log("after reference mRoot...\n");
	}
...
bool BPlusTree::Insert(const mleafdata & _leafdata){
	...
	{
		int itmp = *((int*)data.sKey);
		char ctemp[1000];
		sprintf(ctemp, "[after searchLeafNode:%d]\n", itmp);
		this ->log(ctemp);
	}
...
bool mLeafNode :: dupInsert(const mleafdata & _mleafdata, int _index_insert){
	...
	_tmp_leafdata ->pVal ->add_iVal(0, _mleafdata.pVal ->Term[0]);
...
void BuildTree()
{
	BPlusTree * ptree1, * ptree2, * ptree3, * ptree4;
	mleafdata _data1, _data2, _data3, _data4;
	int  nkey1 = 0, nkey2 = 0, nkey3 = 0, nkey4 = 0;
	bool b1 = false, b2 = false, b3 = false, b4 = false;

	clock_t _total_begin = clock();
	clock_t t_begin, t_end, t_middle;
	t_begin = clock();
	t_end = clock();
	...
	char _row_str[6000];
	char _str[6][600] = {};
	bool sID_sub_dup = false;
	bool fileinput_ok = false;
	...
	ptree1 = new BPlusTree(filePath_sID2s.c_str(), "build");
	ptree2 = new BPlusTree(filePath_s2sID.c_str(), "build");
	ptree3 = new BPlusTree(filePath_o2sID.c_str(), "build");
	ptree4 = new BPlusTree(filePath_opID2sID.c_str(), "build");
	...
	while(!feof(ifp))
	{
		fileinput_ok = DATAINPUT(ifp, _row_str, _str);
		if(!fileinput_ok)
		{
			cout << "total _ii = " << _ii << endl;
			break;
		}
		...
		*(int*)s2i_sID    = i_sID;
		s2i_sID[4] = '\0';
		s2i_pID[4] = '\0';
		if(pre_sID == i_sID)	{	sID_sub_dup = true;	}
		else					{	sID_sub_dup = false;}
		...
		{	// for data1
			...
			_data1.mleafdata_Initial();
			_data1.set_sKey_sValue(_str[0],(int)strlen(_str[0]), _str[3], (int)strlen(_str[3]));

			b1 = ptree1 ->Insert(_data1);
			_data1.cleardata();
			if(b1)				{		nkey1 ++;		}
			if(nkey1 % 1000000 == 0 && b1)
			{
			...
			if((nkey1 % nFlush1 == 0 || nkey1 > level1 && nkey1 % nFlush4 == 0)&& nkey1 > 0 && b1)
			{
				...
				ptree1 ->ClearTree();
				delete ptree1;
				ptree1 = new BPlusTree(filePath_sID2s.c_str(), "open");
			}
			pre_sID = i_sID;
		}
		delete [] s2i_sID;
	...
	ptree1 ->ClearTree();
	t_end = clock();
	cout << "Build tree1 consume: " << (t_end - t_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
	fclose(ifp);
	...
	while(!feof(ifp))
	{
		...
		*(int*)s2i_sID    = i_sID;
		...
		{	// for data2
			...
			_data2.mleafdata_Initial();
			_data2.set_sKey_sValue(_str[3], (int)strlen(_str[3]), s2i_sID, 4);

			b2 = ptree2 ->Insert(_data2);
			if(b2)				{		nkey2 ++;		}
			_data2.cleardata();

			if(nkey2 % 1000000 == 0 && b2)
			{
			...
			if((nkey2 % nFlush1 == 0 || nkey2 > level1 && nkey2 % nFlush4 == 0)&& nkey2 > 0 && b2)
			{
				...
				ptree2 ->ClearTree();
				delete ptree2;
				ptree2 = new BPlusTree(filePath_s2sID.c_str(), "open");
			}
			pre_sID = i_sID;
		}
		delete [] s2i_sID;
	...
	ptree2 ->ClearTree();
	t_end = clock();
	cout << "Build tree2 consume: " << (t_end - t_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
	fclose(ifp);
	...
	while(!feof(ifp))
	{
		...
		*(int*)s2i_sID    = i_sID;
		...
		{	// for data3
			...
			_data3.mleafdata_Initial();
			_data3.set_sKey_sValue(_str[5], (int)strlen(_str[5]), s2i_sID, 4);

			b3 = ptree3 ->Insert(_data3);
			if(b3)				{		nkey3 ++;		}

			_data3.cleardata();

			if(nkey3 % 1000000 == 0 && b3)
			{
			...
			if(((nkey3 % nFlush2 == 0 || nkey3 > level2 && nkey3 % nFlush4 == 0)&& nkey3 > 0)&& b3)
			{
				...
				ptree3 ->ClearTree();
				delete ptree3;
				ptree3 = new BPlusTree(filePath_o2sID.c_str(), "open");
			}
			if(_ii % 300000 == 0)
			{
				cout << "  = >  " << _ii << endl;
				t_middle = t_end;
				t_end = clock();
				cout << "this interval time: " << (t_middle - t_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
				cout << "time consumed up to now: " << (t_end - t_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
			}
		}
	...
	ptree3 ->ClearTree();
	t_end = clock();
	cout << "Build tree3 consume: " << (t_end - t_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
	fclose(ifp);
	...
	while(!feof(ifp))
	{
		...
		*(int*)s2i_sID    = i_sID;
		...
		s2i_pID[4] = '\0';
		{   //  for data4
			//显然data4必须在最后插， 因为破坏了_str[5];
			int len_str5 = (int)strlen(_str[5]);
			memcpy(_str[5] + (int)strlen(_str[5]), s2i_pID, 5);
			_data4.mleafdata_Initial();
			_data4.set_sKey_sValue(_str[5], len_str5 + 4, s2i_sID, 4);

			b4 = ptree4 ->Insert(_data4);
			if(b4)				{		nkey4 ++;		}

			_data4.cleardata();

			if(nkey4 % 1000000 == 0 && b4)
			{
			...
			if(((nkey4 % nFlush2 == 0 || nkey4 > level2 && nkey4 % nFlush4 == 0)&& nkey4 > 0)&&b4)
			{
				...
				ptree4 ->ClearTree();
				delete ptree4;
				ptree4 = new BPlusTree(filePath_opID2sID.c_str(), "open");
			}
	...
	ptree4 ->ClearTree();
	t_end = clock();
	cout << "Build tree4 consume: " << (t_end - t_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
	fclose(ifp);

	clock_t _total_end = clock();
	cout << "total consume: " << (_total_end - _total_begin) / (0.0 +CLOCKS_PER_SEC) << endl;
}
