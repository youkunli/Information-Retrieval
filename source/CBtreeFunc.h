...
public:
	...
	{
		memcpy(_mqueue.qAvailable, qAvailable, sizeof(qAvailable));
		qLenth = _mqueue.qLenth;
	}
	...
	void Push(long long int _iq)
	{
		...
		_iq /= BLOCKSIZE;
		if(qAvailable[_iq] || _iq >= qLenth)// for check
		{
...
//				cout << "block out: " << (lli_tmp) << endl;
	...
	void ReadQueue(char * _queuefile)
	{
		...
		else
		{
			...
			fread(&qLenth, sizeof(int), 1, fp);
			fread(qAvailable, sizeof(bool), qLenth, fp);
//			printf("qLenth = %d\n", qLenth);
		}
		fclose(fp);
	}
	...
	void WriteQueue(char * _queuefile)
	{
		...
		else
		{
			...
			fwrite(&qLenth, sizeof(int), 1, fp);
			fwrite(qAvailable, sizeof(bool), qLenth, fp);
		}
		fclose(fp);
	}
